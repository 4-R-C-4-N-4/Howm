...
function fetchPeers() {
  daemonFetch('/node/peers').then(function (data) {
...
function loadConversations() {
  apiFetch('/conversations').then(function (convos) {
  ...
  apiFetch('/conversations').then(function (convos) {
    ...
    if (total !== lastUnread) {
      ...
      daemonFetch('/notifications/badge', {
        method: 'POST',
        body: JSON.stringify({ capability: 'social.messaging', count: total }),
...
function loadMessages() {
  ...
  apiFetch('/conversations/' + encodeURIComponent(currentPeerId) + '?limit=100').then(function (data) {
    if (!data || !data.messages) return;

    var serverMsgs = data.messages;
    // Merge with optimistic messages not yet confirmed
    var serverIds = {};
    for (var i = 0; i < serverMsgs.length; i++) serverIds[serverMsgs[i].msg_id] = true;
    var remaining = [];
    for (var j = 0; j < optimisticMessages.length; j++) {
      if (!serverIds[optimisticMessages[j].msg_id]) remaining.push(optimisticMessages[j]);
    }
    optimisticMessages = remaining;

    var allMessages = serverMsgs.concat(optimisticMessages);
    allMessages.sort(function (a, b) { return a.sent_at - b.sent_at; });
    ...
    for (var k = 0; k < allMessages.length; k++) {
      ...
      var status = '';
      if (m.direction === 'sent') {
        if (m.delivery_status === 'pending') status = ' <span class="status pending">⏳</span>';
        else if (m.delivery_status === 'delivered') status = ' <span class="status delivered">✓</span>';
        else if (m.delivery_status === 'failed') status = ' <span class="status failed">⚠</span>';
      }
...
function markRead(peerId) {
  apiFetch('/conversations/' + encodeURIComponent(peerId) + '/read', { method: 'POST' })
